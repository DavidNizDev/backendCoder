      ...
      .map((message) => {
        return `<div class="border rounded p-4 w-100 d-flex justify-content-center flex-column gap-4">
                  <div class="d-flex justify-content-center gap-3">
                    ...
                    <div class="w-100">
                      <h4 class="fw-semibold" style="font-size: 1rem;">${message.email
          }:</h4>
                      <p class="fw-lighter m-0 pb-1">${message.message}</p>
                      <div class="d-flex justify-content-between align-items-center">
                        <span style="font-size: .875rem; color: rgba(0,0,0,0.4);">${message.hour
          }</span>
                        <div class="d-flex gap-2">
                          <a class="text-decoration-none" data-bs-toggle="collapse" href="#collapse${message._id
          }" role="button" aria-expanded="false" aria-controls="collapse${message._id
          }"><i class="fa-solid fa-comment message-options__button"></i><span class="comments_counter">${message.replies.length
          }</span></a>
            ...
            : `<div class="border rounded p-4">
                  ...
                  <div>
                    <form action="/consultas/${message._id
          }/reply" method="POST" class="d-flex justify-content-between align-items-center gap-3">
                      <input type="hidden" id="message-id" name="id" value="${message._id
          }" />
                      <input type="hidden" id="user-id" name="userId" value="${message.userId
          }" />
